...
def updatePiLTSP():
    ...
    print("")
    download = True
    if not downloadFile("http://bit.ly/piltspupdate", "/usr/local/bin/Pi_ltsp"):
        download = False
    if not downloadFile(RawRepository + "/master/Scripts/Pi_ltsp-functions-python.py", "/usr/local/bin/Pi_ltsp-functions-python.py"):
        download = False
    if download:
        print("----------------------")
